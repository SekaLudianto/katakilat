      ...
      "entri": [
        {
          ...
          "makna": [
            {
              "kelas": [
                { "kode": "n", "nama": "Nomina", "deskripsi": "kata benda" },
                { "kode": "Astron", "nama": "Astronomi", "deskripsi": "-" }
              ],
              "submakna": ["pertumbuhan objek masif melalui penarikan materi melalui gravitasi"]
            }
          ]
        }
      ]
...
const PAGE_DISPLAY_MS = 4000; // Waktu tampil per halaman pemenang

const MODE_CLASSIC = 'classic';
const MODE_DEF = 'definition';
const MODE_WORDLE = 'wordle_auto';
const MODE_SCRAMBLE = 'scramble';
const MODE_RARE = 'rare_letter';
const MODE_VOWEL = 'single_vowel';
const MODE_CATEGORY = 'category'; // Tebak kata spesifik dari kategori
const MODE_CATEGORY_RUSH = 'category_rush'; // Bebas sebut kata dalam kategori
...
export default function App() {
  ...
  const leaderboardInputRef = useRef(null); // Ref untuk import leaderboard
  ...
  const currentAnswerRef = useRef({ 
      ...
      exact: '', 
      targetChar: '',
      validSet: new Set(), // Untuk Mode Category Rush
      isPlaying: false 
  });
  const roundWinnersRef = useRef(new Set());
  const answeredWordsRef = useRef(new Set()); 
  ...
  const exportLeaderboard = () => {
      const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(globalLeaderboard));
      const downloadAnchorNode = document.createElement('a');
      downloadAnchorNode.setAttribute("href", dataStr);
      downloadAnchorNode.setAttribute("download", `katakilat_lb_${new Date().toISOString().slice(0,10)}.json`);
      document.body.appendChild(downloadAnchorNode);
      downloadAnchorNode.click();
      downloadAnchorNode.remove();
      showNotification("Leaderboard berhasil disimpan (Export)!", "success");
  };
  ...
  const importLeaderboard = (e) => {
      ...
      reader.onload = (ev) => {
          try {
              ...
              setGlobalLeaderboard(json);
              localStorage.setItem('katakilat_leaderboard', JSON.stringify(json));
              showNotification("Leaderboard berhasil dimuat (Import)!", "success");
          } catch(err) {
              console.error(err);
              showNotification("Gagal memuat file leaderboard!", "info");
          }
      }
  ...
  const simulateRoundWinners = () => {
      ...
      setTargetWordObj({ word: "simulasi", display: "simulasi", def: "metode pelatihan yang meragakan sesuatu dalam bentuk tiruan yang mirip dengan keadaan yang sesungguhnya", scrambled: "ASIMULIS" });
      setClassicStats({ total: 15, words: ["contoh", "lain", "kata", "test"] });
      setGameState('round_result');
      
      currentAnswerRef.current.mode = MODE_CLASSIC;
      currentAnswerRef.current.exact = "simulasi";
  ...
  useEffect(() => {
    let interval;
    if (gameState === 'round_result' && gameMode === MODE_CLASSIC && classicStats.words && classicStats.words.length > 0) {
  ...
  // --- ANIMASI ROTASI PAGE PEMENANG (ROUND RESULT) ---
  useEffect(() => {
    let interval;
  ...
  // --- ANIMASI ROTASI PAGE LEADERBOARD & AUTO RESTART (GAME OVER) ---
  useEffect(() => {
    let pageInterval;
    let restartInterval;
  ...
  const handleIncomingChat = (data) => {
      ...
      // LOGIKA VALIDASI PER MODE
      if (currentState.mode === MODE_CLASSIC) {
          const s = currentState.start.toLowerCase();
          const e = currentState.end.toLowerCase();
          const isWordTaken = (word) => answeredWordsRef.current.has(word);

          if (strippedInput.length >= 3 && strippedInput.startsWith(s) && strippedInput.endsWith(e) && dictionary[strippedInput]) {
              if (!isWordTaken(strippedInput)) {
                  isCorrect = true; points = strippedInput.length; finalAnswer = strippedInput;
                  answeredWordsRef.current.add(strippedInput); 
              }
          }
          else {
              const validWord = words.find(w => w.length >= 3 && w.startsWith(s) && w.endsWith(e) && dictionary[w]);
              if (validWord) {
                  if (!isWordTaken(validWord)) {
                      isCorrect = true; points = validWord.length; finalAnswer = validWord;
                      answeredWordsRef.current.add(validWord); 
                  }
              }
          }
      }
      else if (currentState.mode === MODE_CATEGORY_RUSH) {
           const isWordTaken = (word) => answeredWordsRef.current.has(word);
           // ValidSet sudah berisi semua kata yang valid untuk kategori tersebut
           if (currentState.validSet && currentState.validSet.has(strippedInput)) {
               if (!isWordTaken(strippedInput)) {
                   isCorrect = true; points = strippedInput.length * 2; finalAnswer = strippedInput;
                   answeredWordsRef.current.add(strippedInput);
               }
           }
      }
      else if (currentState.mode === MODE_RARE) {
          const targetChar = currentState.targetChar.toLowerCase();
          if (strippedInput.includes(targetChar) && dictionary[strippedInput]) {
              if (!answeredWordsRef.current.has(strippedInput)) {
                  isCorrect = true; points = strippedInput.length * 2; finalAnswer = strippedInput;
                  answeredWordsRef.current.add(strippedInput);
              }
          }
      }
      else if (currentState.mode === MODE_VOWEL) {
          const targetVowel = currentState.targetChar.toLowerCase();
          const allVowels = ['a', 'i', 'u', 'e', 'o'];
          const otherVowels = allVowels.filter(v => v !== targetVowel);
          
          if (dictionary[strippedInput] && strippedInput.includes(targetVowel) && !otherVowels.some(v => strippedInput.includes(v))) {
               if (!answeredWordsRef.current.has(strippedInput)) {
                  isCorrect = true; points = strippedInput.length * 2; finalAnswer = strippedInput;
                  answeredWordsRef.current.add(strippedInput);
              }
          ...
          // MODE TARGET PASTI (Definisi, Scramble, Wordle, CATEGORY SINGLE)
          const target = currentState.exact.toLowerCase();
          if (strippedInput === target) {
              isCorrect = true; points = target.length; finalAnswer = target;
          }
  ...
  const startUltimateChallenge = () => {
      ...
      let queue = [
          ...
          ...Array(2).fill(MODE_CATEGORY_RUSH), // Ganti Category biasa dengan Category Rush
          ...Array(2).fill(MODE_RARE),
          ...Array(2).fill(MODE_DEF),
          ...Array(2).fill(MODE_VOWEL),
          ...Array(2).fill(MODE_WORDLE),
          ...Array(2).fill(MODE_SCRAMBLE)
      ];
  ...
  const prepareRound = (mode) => {
      ...
      answeredWordsRef.current = new Set(); 
      ...
      setTimeout(() => {
          if (mode === MODE_CLASSIC) {
               ...
               while(safety < 50) {
                    ...
                    if (w && w.length >= 3) { selectedWord = w; break; }
                    safety++;
               }
               const s = selectedWord.charAt(0).toLowerCase();
               const e = selectedWord.charAt(selectedWord.length-1).toLowerCase();
               
               const matchingWords = availableWords.filter(w => w.toLowerCase().startsWith(s) && w.toLowerCase().endsWith(e));
          ...
          } else if (mode === MODE_RARE) {
               const rareChars = ['q','x','z','v','f','y'];
               const validRares = rareChars.filter(char => availableWords.some(w => w.includes(char)));
               
               const targetChar = validRares.length > 0 
                   ? validRares[Math.floor(Math.random() * validRares.length)] 
                   : 'k'; 

               setStartLetter(targetChar.toUpperCase()); 
               currentAnswerRef.current.targetChar = targetChar;
               
               speakGoogle(`Cari kata yang mengandung huruf ${targetChar.toUpperCase()}`);

          } else if (mode === MODE_VOWEL) {
               const vowels = ['a','i','u','e','o'];
               let targetVowel = 'a';
               let safety = 0;
               while(safety < 20) {
                   const v = vowels[Math.floor(Math.random() * vowels.length)];
                   const hasWord = availableWords.some(w => {
                       const otherVowels = vowels.filter(x => x !== v);
                       return w.includes(v) && !otherVowels.some(ov => w.includes(ov));
                   });
                   if (hasWord) { targetVowel = v; break; }
                   safety++;
               }

               setStartLetter(targetVowel.toUpperCase());
               currentAnswerRef.current.targetChar = targetVowel;
               
               speakGoogle(`Cari kata yang hanya menggunakan vokal ${targetVowel.toUpperCase()}`);

          } else if (mode === MODE_CATEGORY || mode === MODE_CATEGORY_RUSH) {
               // LOGIC MODE KATEGORI BARU
               const excludedClasses = ['Nomina', 'Verba', 'Adjektiva', 'Adverbia', 'Numeralia', 'Pronomina', 'Partikel', 'Konjungsi', 'Preposisi', 'Interjeksi', 'Lain-lain'];
               
               // Build category map on the fly (better pre-calc in real app)
               let possibleCategories = {};
               
               // Random sample search for efficiency
               for(let i=0; i<300; i++) {
                   const w = availableWords[Math.floor(Math.random() * availableWords.length)];
                   const entry = dictionary[w]?.data?.entri?.[0];
                   const meanings = entry?.makna || [];
                   for (let m of meanings) {
                       if (m.kelas) {
                           const catClass = m.kelas.find(c => !excludedClasses.includes(c.nama));
                           if (catClass) {
                               if(!possibleCategories[catClass.nama]) possibleCategories[catClass.nama] = [];
                               possibleCategories[catClass.nama].push(w);
                           }
                       }
                   }
               }
               
               const validCats = Object.keys(possibleCategories).filter(k => possibleCategories[k].length > 0);
               let selectedCat = 'Astronomi'; // Fallback
               let validWords = ['akresi'];
               
               if (validCats.length > 0) {
                   selectedCat = validCats[Math.floor(Math.random() * validCats.length)];
                   validWords = possibleCategories[selectedCat];
               }

               if (mode === MODE_CATEGORY_RUSH) {
                   // Category Rush: Multiple words valid
                   setTargetWordObj({ category: selectedCat });
                   currentAnswerRef.current.validSet = new Set(validWords); // All words in this cat are valid
                   speakGoogle(`Sebutkan kata yang berhubungan dengan kategori ${selectedCat}`);
               } else {
                   // Single Target (Old Mode Category)
                   const selectedWord = validWords[Math.floor(Math.random() * validWords.length)];
                   const entry = dictionary[selectedWord]?.data?.entri?.[0];
                   let rawDef = entry?.makna?.[0]?.submakna?.[0] || "-";
                   if(typeof rawDef === 'object') rawDef = JSON.stringify(rawDef);
                   const def = String(rawDef);

                   setTargetWordObj({ word: selectedWord, display: selectedWord, def, category: selectedCat });
                   currentAnswerRef.current.exact = selectedWord;
                   speakGoogle(`Tebak istilah di bidang ${selectedCat} berikut ini.`);
               }

          } else {
               // Logic Wordle/Def/Scramble
               ...
               const entry = dictionary[selectedWord]?.data?.entri?.[0];
               let rawDef = entry?.makna?.[0]?.submakna?.[0] || "Definisi tidak ditemukan";
               if (typeof rawDef === 'object') rawDef = JSON.stringify(rawDef);
               const def = String(rawDef);
               ...
               setTargetWordObj({ word: selectedWord, display: selectedWord, def, scrambled, origin });
               currentAnswerRef.current.exact = selectedWord;
  ...
  const endRound = (forceEnd = false) => {
      ...
      const winnerCount = roundWinnersRef.current.size;
  ...
  // --- EFFECT TIMER RESULT & AUTO NEXT (HANDLE PAUSE & DURATION) ---
  useEffect(() => {
      let timeout;
      if (gameState === 'round_result' && !isPaused) {
          // Calculate duration based on winner pages
          const winnerCount = roundWinners.length;
          const itemsPerPage = 5;
          const pages = Math.ceil(winnerCount / itemsPerPage) || 1;
          const duration = Math.max(pages * 4000, 5000); // 4 sec per page, min 5 sec
  ...
  }, [gameState, isPaused, roundWinners]); 
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-center space-y-8">
                ...
                <div className={`mt-auto p-4 rounded-xl text-left w-full border text-xs space-y-3 ${isDarkMode ? 'bg-slate-900/50 border-slate-700 text-slate-400' : 'bg-white/50 border-indigo-200 text-slate-600'}`}>
                    ...
                    <div className="w-full grid grid-cols-2 gap-2 mt-2">
                        <button onClick={exportLeaderboard} className="flex items-center justify-center gap-2 py-2 bg-indigo-500/20 text-indigo-400 rounded-lg hover:bg-indigo-500/30 text-xs font-bold transition-all border border-indigo-500/30">
                            <Download className="w-3 h-3" /> Export LB
                        </button>
                        <button onClick={() => leaderboardInputRef.current.click()} className="flex items-center justify-center gap-2 py-2 bg-pink-500/20 text-pink-400 rounded-lg hover:bg-pink-500/30 text-xs font-bold transition-all border border-pink-500/30">
                            <Upload className="w-3 h-3" /> Import LB
                        </button>
                        <input 
                            type="file" 
            ...
            <div className="flex-1 flex flex-col relative overflow-hidden">
                <div className="flex justify-between items-center mb-4">
                    <div className={`px-3 py-1.5 rounded-lg border ${isDarkMode ? 'bg-slate-800 border-slate-700' : 'bg-white/50 border-indigo-200'}`}>
                        <span className={`block text-[10px] font-bold uppercase tracking-wider ${themeClasses.textDim}`}>Ronde {challengeIndex+1}/16</span>
                        <span className={`block text-sm font-black ${themeClasses.textMain}`}>
                            {gameMode === MODE_CLASSIC ? 'AWAL-AKHIR' : 
                             gameMode === MODE_DEF ? 'TEBAK KATA' : 
                             gameMode === MODE_WORDLE ? 'WORDLE' : 
                             gameMode === MODE_SCRAMBLE ? 'ANAGRAM' :
                             gameMode === MODE_CATEGORY_RUSH ? 'KATEGORI BEBAS' :
                             gameMode === MODE_CATEGORY ? 'TEBAK ISTILAH' :
                             gameMode === MODE_RARE ? 'HURUF LANGKA' : 'SATU VOKAL'}
                        </span>
                    </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-start pt-0 relative w-full mb-20">
                    ...
                    {gameMode === MODE_CLASSIC && (
                        <div className="flex flex-col items-center w-full animate-in fade-in zoom-in duration-500 mt-2">
                             ...
                             <div className={`mt-4 mb-2 flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold border transition-colors ${isDarkMode ? 'bg-blue-900/30 border-blue-800 text-blue-300' : 'bg-blue-50 border-blue-200 text-blue-600'}`}>
                                <Database className="w-3 h-3" />
                                <span>{classicStats.total} Kata Tersedia</span>
                             </div>
                             <div className={`mt-2 flex items-center gap-2 px-4 py-2 rounded-xl border border-dashed ${isDarkMode ? 'border-white/10 bg-white/5' : 'border-indigo-200 bg-indigo-50/50'}`}>
                                 <Info className={`w-4 h-4 ${themeClasses.textDim}`} />
                                 <span className={`text-xs font-medium ${themeClasses.textDim}`}>
                                     Ketik kata dengan <b>Awalan</b> & <b>Akhiran</b> di atas!<br/>
                                     <span className="opacity-75">(Satu kata = 1 Pemenang tercepat)</span>
                                 </span>
                             </div>
                        </div>
                    )}
                    ...
                    {gameMode === MODE_RARE && (
                        <div className="flex flex-col items-center w-full animate-in fade-in zoom-in duration-500 mt-2">
                             <span className={`text-[10px] font-bold tracking-widest mb-2 ${themeClasses.textDim}`}>CARI KATA YANG MENGANDUNG HURUF</span>
                             <div className={`w-32 h-36 flex items-center justify-center text-8xl font-black rounded-3xl mb-4 border-4 ${isDarkMode ? 'bg-purple-900/40 border-purple-500 text-purple-400 shadow-[0_0_30px_rgba(168,85,247,0.3)]' : 'bg-purple-50 border-purple-300 text-purple-600'}`}>
                                 {startLetter}
                             </div>
                             <div className={`mt-2 px-4 py-2 rounded-xl border ${isDarkMode ? 'border-purple-500/30 bg-purple-900/20 text-purple-200' : 'border-purple-200 bg-purple-50 text-purple-700'}`}>
                                 <div className="flex items-center gap-2">
                                     <Gem className="w-4 h-4" />
                                     <span className="text-xs font-bold">Bonus Poin x2</span>
                                 </div>
                             </div>
                        </div>
                    )}
                    ...
                    {gameMode === MODE_VOWEL && (
                        <div className="flex flex-col items-center w-full animate-in fade-in zoom-in duration-500 mt-2">
                             <span className={`text-[10px] font-bold tracking-widest mb-2 ${themeClasses.textDim}`}>HANYA BOLEH ADA VOKAL</span>
                             <div className={`w-32 h-36 flex items-center justify-center text-8xl font-black rounded-3xl mb-4 border-4 ${isDarkMode ? 'bg-orange-900/40 border-orange-500 text-orange-400 shadow-[0_0_30px_rgba(249,115,22,0.3)]' : 'bg-orange-50 border-orange-300 text-orange-600'}`}>
                                 {startLetter}
                             </div>
                             <div className={`mt-2 px-4 py-2 rounded-xl border ${isDarkMode ? 'border-orange-500/30 bg-orange-900/20 text-orange-200' : 'border-orange-200 bg-orange-50 text-orange-700'}`}>
                                 <div className="flex items-center gap-2">
                                     <Filter className="w-4 h-4" />
                                     <span className="text-xs font-bold">Contoh: M{startLetter}K{startLetter}N</span>
                                 </div>
                             </div>
                        </div>
                    )}

                    {(gameMode === MODE_DEF || gameMode === MODE_CATEGORY || gameMode === MODE_CATEGORY_RUSH) && targetWordObj && (
                        <div className={`w-full p-4 rounded-2xl border text-center animate-in slide-in-from-bottom duration-500 mt-0 ${themeClasses.defBox}`}>
                            <span className={`text-xs font-black uppercase tracking-widest mb-2 block ${themeClasses.accent}`}>
                                {gameMode === MODE_CATEGORY_RUSH ? 'KATEGORI (BEBAS SEBUT)' : (gameMode === MODE_CATEGORY ? 'TEBAK ISTILAH' : 'TEBAK KATA')}
                            </span>
                            
                            {gameMode === MODE_CATEGORY_RUSH ? (
                                <div className="py-4">
                                     <h2 className={`text-4xl font-black uppercase tracking-wide ${themeClasses.textMain}`}>{targetWordObj.category}</h2>
                                     <p className={`text-xs mt-2 ${themeClasses.textDim}`}>Sebutkan kata apapun yang berhubungan!</p>
                                     <p className="text-[10px] opacity-60 mt-1">(1 Kata = 1 Pemenang Tercepat)</p>
                                </div>
                            ) : (
                                <>
                                    <p className="text-lg md:text-xl font-serif italic leading-relaxed mb-4">"{targetWordObj.def}"</p>
                                    {targetWordObj.category && (
                                        <div className="mb-3">
                                            <span className={`text-[10px] px-2 py-0.5 rounded border border-blue-500/30 text-blue-400 bg-blue-500/10`}>
                                                Bidang: {targetWordObj.category}
                                            </span>
                                        </div>
                                    )}
                ...
                >
                    <div className="w-full space-y-2 relative group">
                        <div className={`w-full flex justify-center items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity ${isDragging ? 'opacity-100' : ''}`}>
                            <button onClick={() => setChatScale(prev => prev === 1 ? 0.7 : 1)} className="bg-black/30 backdrop-blur-sm rounded-full p-1 border border-white/10 hover:bg-black/50 transition-colors">
            ...
            <div className="absolute inset-0 z-40 backdrop-blur-md bg-black/50 flex flex-col items-center justify-center p-6 animate-in fade-in duration-300 rounded-3xl">
                {isPaused ? (
                    ...
                    <div className={`w-full max-w-sm rounded-3xl p-6 text-center shadow-2xl border-2 relative ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white/90 border-indigo-200'}`}>
                        {(gameMode === MODE_SCRAMBLE || gameMode === MODE_WORDLE || gameMode === MODE_DEF || gameMode === MODE_CATEGORY || gameMode === MODE_CATEGORY_RUSH) && (
                        ...
                        <div className={`text-3xl font-black uppercase tracking-wider ${themeClasses.accent}`}>
                            {gameMode === MODE_CATEGORY_RUSH ? targetWordObj?.category : (targetWordObj?.word || (currentAnswerRef.current.exact || '...'))}
                        </div>
                                        ...
                                        <div key={idx} className={`flex items-center justify-between p-2 rounded-lg border ${themeClasses.winnerCard}`}> 
                                            <div className="flex items-center gap-2"> 
                                                ...
                                                <div className="flex flex-col items-start overflow-hidden">
                                                    ...
                                                    <span className={`text-[10px] italic truncate max-w-[120px] ${isDarkMode ? 'text-emerald-400/70' : 'text-emerald-600/70'}`}>
                                                        "{winner.answer ? winner.answer.toLowerCase() : ''}"
                                                    </span>
                                                </div>
                                            </div>
                        ...
                        <div className="flex flex-col items-center mt-4">
                            <span className={`text-xs font-bold ${themeClasses.textDim} animate-pulse`}>Lanjut otomatis...</span>
                        </div>
                    </div>
                )}
            </div>
